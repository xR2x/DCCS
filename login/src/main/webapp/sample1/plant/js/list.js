var base = {
	pageSize:25,
	sort:(hash.get("sort"))?hash.get("sort"):"plant",
	...
	init:function(){
		...
		list.set(base.columns,{excel:{api:"master/plant/read",fileName:main.nowMenu.name}});
		base.load();
	},
	load:function(){
		var param = {
	...
	makeData:function(json){
		...
		for(var i=0;i<json.length;i++){
			...
			if(Number(data.use_count)>0){
				...
				result[i].manage = "<span class='btn-group'><button class='btn middle' seq='"+data.seq+"' onclick='base.view(this)'>조회</button><button class='btn middle' seq='"+data.seq+"' onclick='base.modify(this)'>수정</button><button class='btn middle color red' seq='"+data.seq+"' plant='"+data.plant+"' plant_name='"+data.plant_name+"' onclick='base.remove_confirm(this)'>삭제</button></span>";
			}
		}
		return result;
	},
	use_info:function(){
		$.alert("사용중인 Plant는 <br>삭제할 수 없습니다.");
	},
	view:function(obj){
		hash.set({"service":"master/plant/view&seq="+$(obj).attr("seq")});
	},
	modify:function(obj){
		hash.set({"service":"master/plant/update&seq="+$(obj).attr("seq")});
	},
	remove_confirm:function(obj){
		var plant_name = $(obj).attr("plant_name");
		popup.create({
			title:"Plant 삭제"
			,width:350
			,height:160
			,type:"confirm"
			,message:"Plant : "+plant_name+"을(를) 삭제하시겠습니까?"
			,button:[{
