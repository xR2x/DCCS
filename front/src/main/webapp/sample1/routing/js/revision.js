var base = {
				...
				try{
					createForm.list.create({
						...
						,columns:[
							...
							,{header:'변경사유',name:'remark',resizable: false,type:"text"}
							...
							,{header:'삭제',name:'delete',resizable: false,width:70,type:"customize",html:"<button class='btn middle color red' onclick='base.del(this)'>삭제</button>",attr:["seq","active_flag","revision"]}
						]
					});
					setTimeout(function(){
						base.make();
				...
				title:"<strong>네</strong> (삭제)"
					...
					,handler:function(){
						...
						$.ajax({ 
							type:"POST",
							dataType:"json",
							url:"/master/routing/deleteRevision",
							data:param,
							success:function(json, status, res){
								try{
			...
			,button:[{
				title:"<strong>네</strong> (변경)"
					...
					,handler:function(){
						...
						$(".tui-grid-cell[data-column-name=delete] button").prop("disabled",false);
	...
	activeChange:function(obj){
		...
		$.ajax({ 
			type:"POST",
			dataType:"json",
			url:"/master/routing/activeRevision",
			data:param,
			success:function(json, status, res){
				try{
					$(obj).prop("checked",true);
