var base = {
	...
	init:function(){
		...
		base.columns = [{
				title:"참조 Routing 이름",id:"material",
		...
		var addSheet = {
			"item":[
				{"title":"참조 Routing 이름","id":"material","width":"300"}
		...
		list.set(base.columns,{excel:{api:"master/routing/read",addSheet:addSheet,active_flag:"X",standard_flag:"X",fileName:main.nowMenu.name},button:[{position:0,cssClass:"middle color blue",name:"<img src=/front/common/img/icon/reports-stack.png> 선택 Routing Report 출력",handler:base.report}]});
		base.load();
	},
	load:function(){
		var param = {
	...
	makeData:function(json){
		...
		for(var i=0;i<json.length;i++){
			...
			var btnReport = "<button class='btn middle color blue' onclick='base.report(this)'>Report</button>";	   			
	...
	approvalConfirm:function(obj){
		...
		popup.create({
			...
			,height:190
			,type:"confirm"
			,message:"참조 Routing 이름 : "+material+" <br> Revision : "+revision+" <br>을 승인의뢰 하시겠습니까?"
			,button:[{
	...
	report:function(obj){
		...
		if(obj) {					 			
			...
			array[0] = {
				seq:targetObj.attr("seq")
				,material:targetObj.attr("material")
				,revision:targetObj.attr("revision")
			}
		}else {
	...
	view:function(obj){	   
		hash.set({"service":"master/routing/standardView&seq="+$(obj).closest("span").attr("seq")});
	},
	modify:function(obj){
		hash.set({"service":"master/routing/standardUpdate&seq="+$(obj).closest("span").attr("seq")});
	},
	remove_confirm:function(obj){
		...
		popup.create({
			...
			,height:190
			,type:"confirm"
			,message:"참조 Routing 이름 : "+material+" <br> Revision : "+revision+" <br>을 삭제하시겠습니까?"
			,button:[{
